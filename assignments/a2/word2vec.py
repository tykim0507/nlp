...
def negSamplingLossAndGradient(
    ...
    index_list, count_list = np.unique(indices, return_counts = True)
    # count_dict = {index : count for index, count in zip(*np.unique(np.array(indices), return_counts = True))}
    count_vector = np.zeros(outsideVectors.shape[0])
    count_vector[index_list] = count_list

    sigmoid_center_dot_product = sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))
    sigmoid_negative_dot_product = sigmoid(-np.dot(outsideVectors[indices], centerWordVec))

    loss = -np.log(sigmoid_center_dot_product) - np.sum(np.log(sigmoid_negative_dot_product))
    gradCenterVec = -(1 - sigmoid_center_dot_product) * outsideVectors[outsideWordIdx] + np.dot((1 - sigmoid_negative_dot_product), outsideVectors[indices])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideword in outsideWords:
        ...
        loss_increment, gradCenterVec, gradOutsideVectors_increment = word2vecLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += loss_increment
        gradCenterVecs[word2Ind[currentCenterWord]]+= gradCenterVec
        gradOutsideVectors += gradOutsideVectors_increment
