...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        flat_batch_of_word_indices = torch.flatten(w)
        flat_x = torch.index_select(self.embeddings, 0, flat_batch_of_word_indices)
        x = flat_x.view((w.shape[0], -1))
    ...
    def forward(self, w):
        ...
        hidden = self.dropout(torch.relu(torch.matmul(self.embedding_lookup(w),self.embed_to_hidden_weight) + self.embed_to_hidden_bias))
